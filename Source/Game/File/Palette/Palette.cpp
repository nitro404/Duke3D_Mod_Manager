...
Colour Palette::lookupColour(uint8_t colourIndex, uint8_t colourTableIndex, bool * error) const {
...
std::vector<Colour> Palette::getColourTableData(uint8_t colourTableIndex) const {
	std::vector<Colour> colours(NUMBER_OF_COLOURS);
...
std::vector<Colour> Palette::getAllColourTableData(uint8_t colourTableIndex) const {
	...
	std::vector<Colour> colours(NUMBER_OF_COLOURS * colourTableCount);
	bool error = false;
...
void Palette::addMetadata(std::vector<std::pair<std::string, std::string>> & metadata) const {
	...
	if(colourTableCount != 1) {
		for(uint8_t i = 0; i < colourTableCount; i++) {
			...
			metadata.push_back({ fmt::format("Colour Table #{}", i + 1), colourTableDescription.empty() ? "Untitled" : colourTableDescription });
		}
	}
}
