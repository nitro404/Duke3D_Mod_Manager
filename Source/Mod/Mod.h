...
public:
	...
	static std::unique_ptr<Mod> parseFrom(const rapidjson::Value & modValue);
	static std::unique_ptr<Mod> parseFrom(const tinyxml2::XMLElement * modElement);
	...
	bool isValid() const;
	static bool isValid(const Mod * m);
