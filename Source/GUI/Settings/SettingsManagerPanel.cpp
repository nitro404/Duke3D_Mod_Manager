...
SettingsManagerPanel::SettingsManagerPanel(std::shared_ptr<ModManager> modManager, wxWindow * parent, wxWindowID windowID, const wxPoint & position, const wxSize & size, long style)
	...
	m_settingsPanels.push_back(SettingPanel::createStringSettingPanel(settings->appSymlinkName, SettingsManager::DEFAULT_APP_SYMLINK_NAME, "App Symbolic Link Name", generalSettingsPanel, generalSettingsSizer, 1));
	...
	m_settingsPanels.push_back(SettingPanel::createBooleanSettingPanel(settings->dosboxFullscreen, SettingsManager::DEFAULT_DOSBOX_FULLSCREEN, "Fullscreen", dosboxSettingsPanel, dosboxSettingsSizer));
