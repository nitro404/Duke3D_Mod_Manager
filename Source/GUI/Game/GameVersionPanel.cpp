...
GameVersionPanel::GameVersionPanel(std::shared_ptr<GameVersion> gameVersion, wxWindow * parent, wxWindowID windowID, const wxPoint & position, const wxSize & size, long style)
	...
	m_settingsPanels.push_back(SettingPanel::createStringSettingPanel<void>(std::bind(&GameVersion::getGameConfigurationDirectoryPath, m_gameVersion.get()), std::bind(&GameVersion::setGameConfigurationDirectoryPath, m_gameVersion.get(), std::placeholders::_1), defaultGameVersion != nullptr ? defaultGameVersion->getGameConfigurationDirectoryPath() : "", "Game Configuration Directory Path", gameConfigurationPanel, gameConfigurationSizer));
