...
void GameManagerPanel::onNotebookPageChanged(wxBookCtrlEvent & event) {
	...
	if(settings->segmentAnalyticsEnabled) {
		...
		if(currentGameVersion != nullptr && currentGameVersion->hasID() && currentGameVersion->hasLongName()) {
			...
			properties["gameID"] = currentGameVersion->getID();
			properties["shortName"] = currentGameVersion->getShortName();
			properties["longName"] = currentGameVersion->getLongName();

			if(currentGameVersion->hasInstalledTimePoint()) {
				properties["installedAt"] = Utilities::timePointToString(currentGameVersion->getInstalledTimePoint().value(), Utilities::TimeFormat::ISO8601);
			}

			if(currentGameVersion->hasLastPlayedTimePoint()) {
				properties["lastPlayedAt"] = Utilities::timePointToString(currentGameVersion->getLastPlayedTimePoint().value(), Utilities::TimeFormat::ISO8601);
			}

			properties["hasGamePath"] = currentGameVersion->hasGamePath();
			properties["gameExecutableName"] = currentGameVersion->getGameExecutableName();

			if(currentGameVersion->hasSetupExecutableName()) {
				properties["setupExecutableName"] = currentGameVersion->getSetupExecutableName().value();
			}

			if(currentGameVersion->hasGroupFileInstallPath()) {
				properties["groupFileInstallPath"] = currentGameVersion->getGroupFileInstallPath().value();
			}

			const std::optional<bool> & optionalRequiresCombinedGroup = currentGameVersion->getRequiresCombinedGroup();

			if(optionalRequiresCombinedGroup.has_value()) {
				properties["requiresCombinedGroup"] = optionalRequiresCombinedGroup.value();
			}

			const std::optional<bool> & optionalRequiresGroupFileExtraction = currentGameVersion->getRequiresGroupFileExtraction();

			if(optionalRequiresGroupFileExtraction.has_value()) {
				properties["requiresGroupFileExtraction"] = optionalRequiresGroupFileExtraction.value();
			}

			properties["requiresDOSBox"] = currentGameVersion->doesRequireDOSBox();
			properties["localWorkingDirectory"] = currentGameVersion->hasLocalWorkingDirectory();
			properties["relativeConFilePath"] = currentGameVersion->hasRelativeConFilePath();
			properties["supportsSubdirectories"] = currentGameVersion->doesSupportSubdirectories();

			properties["modDirectoryName"] = currentGameVersion->getModDirectoryName();

			std::optional<bool> optionalWorldTourGroupSupported(currentGameVersion->getWorldTourGroupSupported());

			if(optionalWorldTourGroupSupported.has_value()) {
				properties["worldTourGroupSupported"] = optionalWorldTourGroupSupported.value();
			}

			if(currentGameVersion->hasConFileArgumentFlag()) {
				properties["conFileArgumentFlag"] = currentGameVersion->getConFileArgumentFlag().value();
			}

			if(currentGameVersion->hasExtraConFileArgumentFlag()) {
				properties["extraConFileArgumentFlag"] = currentGameVersion->getExtraConFileArgumentFlag().value();
			}

			if(currentGameVersion->hasGroupFileArgumentFlag()) {
				properties["groupFileArgumentFlag"] = currentGameVersion->getGroupFileArgumentFlag().value();
			}

			if(currentGameVersion->hasDefFileArgumentFlag()) {
				properties["defFileArgumentFlag"] = currentGameVersion->getDefFileArgumentFlag().value();
			}

			if(currentGameVersion->hasExtraDefFileArgumentFlag()) {
				properties["extraDefFileArgumentFlag"] = currentGameVersion->getExtraDefFileArgumentFlag().value();
			}

			if(currentGameVersion->hasMapFileArgumentFlag()) {
				properties["mapFileArgumentFlag"] = currentGameVersion->getMapFileArgumentFlag().value();
			}

			properties["episodeArgumentFlag"] = currentGameVersion->getEpisodeArgumentFlag();
			properties["levelArgumentFlag"] = currentGameVersion->getLevelArgumentFlag();
			properties["skillArgumentFlag"] = currentGameVersion->getSkillArgumentFlag();
			properties["skillStartValue"] = currentGameVersion->getSkillStartValue();
			properties["recordDemoArgumentFlag"] = currentGameVersion->getRecordDemoArgumentFlag();

			if(currentGameVersion->hasPlayDemoArgumentFlag()) {
				properties["playDemoArgumentFlag"] = currentGameVersion->getPlayDemoArgumentFlag().value();
			}

			if(currentGameVersion->hasRespawnModeArgumentFlag()) {
				properties["respawnModeArgumentFlag"] = currentGameVersion->getRespawnModeArgumentFlag().value();
			}

			if(currentGameVersion->hasWeaponSwitchOrderArgumentFlag()) {
				properties["weaponSwitchOrderArgumentFlag"] = currentGameVersion->getWeaponSwitchOrderArgumentFlag().value();
			}

			if(currentGameVersion->hasDisableMonstersArgumentFlag()) {
				properties["disableMonstersArgumentFlag"] = currentGameVersion->getDisableMonstersArgumentFlag().value();
			}

			if(currentGameVersion->hasDisableSoundArgumentFlag()) {
				properties["disableSoundArgumentFlag"] = currentGameVersion->getDisableSoundArgumentFlag().value();
			}

			if(currentGameVersion->hasDisableMusicArgumentFlag()) {
				properties["disableMusicArgumentFlag"] = currentGameVersion->getDisableMusicArgumentFlag().value();
			}

			properties["numberOfSupportedOperatingSystems"] = currentGameVersion->numberOfSupportedOperatingSystems();
			properties["numberOfCompatibleGameVersions"] = currentGameVersion->numberOfCompatibleGameVersions();
